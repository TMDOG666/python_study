...
print(f"编码后的 bytes: {bytes_data}") # result ： b'\xe4\xbd\xa0\xe5\xa5\xbd\xef\xbc\x8cPython' 中文会做处理成二进制
print(f"bytes 的类型是: {type(bytes_data)}")

for byte in bytes_data:
    print(f"{byte} 类型：{type(byte)}", end='')
...
# 解码: 将 bytes 变回字符串
decode_string = bytes_data.decode('utf-8')
print(f"解码后的字符串: {decode_string}")
...
# ord()具体解释​​
# ​​ord() 函数​​
# 是 Python 的内置函数，全称是 "ordinal"（序数）。
# 它接受一个​​长度为 1 的字符​​作为参数，返回该字符对应的 ​​Unicode 码点​​（整数）。
# 对于 ASCII 字符（如字母、数字、符号），返回值就是它的 ASCII 码。
